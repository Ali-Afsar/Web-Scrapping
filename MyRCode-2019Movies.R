...
# Web Data Scraping using R for IMDB Movies rating 2019
...
# Now I found that Movie No 59 and 88 does not have metascore (look visually)

# Try to Fill 'NA' at 59 and 88th position
...
# I found that only 43 Movies has gross earning out of 100. 
#Now fill NA to 57 data

gross_data <- append(gross_data,"NA", after = 0)

for (i in c(3,4,7:14,17:22,24,27:29,32,34:37,39,41,44,45,48:50,52,54,55,57:59,61,67,68,70:72,77,78,81:83,88:90,94,98:100)) {
...
length(gross_data) #(It becomes 102, I dont know how but 1:100 data is correct and 101 and 102 is wrong data so removing it)
...
# Now adding one more column for Year

movies_2019_df$Year <- "2019"

summary(movies_2019_df$Year)

dim(movies_2019_df)

#*****************Binding all the rows from 2016,2017,2018 and 2019 data frame.******************

Imdb_Movies <- rbind(movies_2016_df,movies_2017_df,movies_2018_df,movies_2019_df)

dim(Imdb_Movies)

# Now writing data into excel

install.packages("xlsx")
library(xlsx)

write.xlsx(Imdb_Movies, file = "Movies16-19.xlsx", sheetName = "AllMovies")